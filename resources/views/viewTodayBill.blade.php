            ...
            <div class="row">
               <div class="col-12 col-sm-12  mb-4">
                <div class="card redial-border-light redial-shadow">
                    <div class="card-body">
                        <h6 class="header-title pl-3 redial-relative">Bill Details</h6>
                        <dl class="row mb-0 redial-line-height-2_5">

                            <dt class="col-sm-5">Bill Id:</dt>
                            <dd class="col-sm-7">{{$viewTodayBill['invoice_number']}}</dd>

                            <dt class="col-sm-5">Date:</dt>
                            <dd class="col-sm-7">{{$viewTodayBill['ordered_date']}}</dd>

                            <dt class="col-sm-5">Time:</dt>
                            <dd class="col-sm-7">{{$viewTodayBill['ordered_time']}}</dd>

                            <dt class="col-sm-5">Items:</dt>
                            <dd class="col-sm-7">{{count($viewTodayBill['orders'])}}</dd>

                            <dt class="col-sm-5">Amount:</dt>
                            <dd class="col-sm-7">&#x20B9;{{$viewTodayBill['ordered_total']}}</dd>
                    ...
                    <div class="card redial-border-light redial-shadow">
                        <div class="card-body">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    @foreach($viewTodayBill['orders'] as $orders)
                                    <tr> 
                                        <td>{{$no++}}</td>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right">&#x20B9;{{$viewTodayBill['ordered_total']}}</td>
                                    </tr>
                                </tbody>
                            </table>
...
<div id="componentID" style="display: none;">
    <div class="row">President Cafe</div><div class="row"> Bill No.: {{$viewTodayBill['invoice_number']}}</div><div class="row"> {{$viewTodayBill['ordered_date']}}  {{$viewTodayBill['ordered_time']}}</div><div class="row"> Total Items: {{$viewTodayBill['ordered_count']}}</div><table> <tr>  <td colspan="4">   -----------------------------------------------------------</td></tr><tr>  <td>S No</td>  <td>Item</td> <td>Price</td> <td>Qty</td>  <td>Amt</td></tr><tr>  <td colspan="4">   ----------------------------------------------------------- </td></tr>
        @for($i=0; $i < count($viewTodayBill['orders']); $i++)
        <tr><td>{{$i+1}}</td>  <td>{{$viewTodayBill['orders'][$i]['order_name']}}</td> <td>{{$viewTodayBill['orders'][$i]['order_price']}}rs</td> <td>{{$viewTodayBill['orders'][$i]['order_quantity']}}</td> <td>{{$viewTodayBill['orders'][$i]['total_price']}}</td></tr>  
        @endfor
        <td colspan="4">   ---------------------------------------------------------- </td></tr><tr>  <td></td>  <td></td>  <td style="font-weight: bolder">Grand Total = </td>  <td>{{$viewTodayBill['ordered_total']}}</td></tr><tr>  <td colspan="4">   ----------------------------------------------------------- </td></tr></table>
    </div>
